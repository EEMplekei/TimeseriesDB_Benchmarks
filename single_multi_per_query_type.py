...
# get file parameters and return the mean of the file for each query type (a list of 11)
...
for i, db_size in zip(range(3), dataset_size):

	bar1 = ax.bar(index, list(map(lambda x: x/1000, results_single[('influx', db_size)])),
					 bar_width, label='influx single node', color=color_map['influx_single'])

	bar2 = ax.bar([i + bar_width for i in index],
					 list(map(lambda x: x/1000, results_multi[('influx', db_size)])), bar_width, label='influx multi node', color=color_map['influx_multi'])

	bar3 = ax.bar([i + 2 * bar_width for i in index],
					 list(map(lambda x: x/1000, results_single[('timescale', db_size)])), bar_width, label='timescale single node', color=color_map['timescale_single'])

	bar4 = ax.bar([i + 3 * bar_width for i in index],
					 list(map(lambda x: x/1000, results_multi[('timescale', db_size)])), bar_width, label='timescale multi node', color=color_map['timescale_multi'])
	...
	ax.set_xticklabels(query_types, rotation=20)
	ax.legend(framealpha=0)

	# Add labels above each bar
	for bar1, bar2, bar3, bar4, value_inf_s, value_inf_m, value_time_s, value_time_m in zip(bar1, bar2, bar3, bar4, results_single[('influx', db_size)], results_multi[('influx', db_size)], results_single[('timescale', db_size)], results_multi[('timescale', db_size)]):

		ax.text(bar1.get_x() + bar1.get_width() / 2, bar1.get_height(), f'{value_inf_s/1000:.1f}', ha='center', va='bottom')

		ax.text(bar2.get_x() + bar2.get_width() / 2, bar2.get_height(), f'{value_inf_m/1000:.1f}', ha='center', va='bottom')
		
		ax.text(bar3.get_x() + bar3.get_width() / 2, bar3.get_height(), f'{value_time_s/1000:.1f}', ha='center', va='bottom')
		
		ax.text(bar4.get_x() + bar4.get_width() / 2, bar4.get_height(), f'{value_time_m/1000:.1f}', ha='center', va='bottom')
